...
class Block:
    __slots__ = ('_M', 'x', 'y', 'num', 'gene', 'value', 'cell')
    ...
    def insert(self, gene, x, y, value, cell):
        self.num += len(gene)
...
class Data:
    def __init__(self, gene, x, y, value, cell):
    ...
    def sort(self):
        ...
        self.cell = np.asarray(self.cell, dtype=np.uint8)
        ...
        self.cell = self.cell[sorted_idx]
    ...
    def block(self):
        ...
        for y in self.sorted_idx_part:
            ...
            cell_range = self.cell[idx:idx+num]
            idx += num
            blocks_num[t] = (num-1)//block.M
            t += 1
            for i in range(0, num, block.M):
                block = Block()
                block.insert(gene_range[i:i+block.M], x_range[i:i+block.M], y_range[i:i+block.M], value_range[i:i+block.M], cell_range[i:i+block.M])
                blocks.insert(block)
        return blocks, blocks_num

    def printf(self, blocks, blocks_num, data_file, idx_file, gene_file, cell_file):
        dt = open(data_file, 'w')
        ...
        for block in blocks:
            ...
            for i in range(block.num):
                ...
                if cellDict[block.cell[i]] is None:
                    cellDict[block.cell[i]] = [last_b, bit-1]
                else:
                    cellDict[block.cell[i]].extend([last_b, bit-1])
        ...
        print(','.join(list(map(str, cell_list))), file=cl)
        for cell in cell_list:
            print(','.join(map(str, cellDict[cell])), file=cl)
        del cell_list, cellDict
        gc.collect()
...
def load_data(file):
    ...
    ipt.readline()
    point = ipt.readline()
    gene, x, y, value, cell = point.strip().split('\t')
    data = Data(gene, int(x), int(y), value, cell)
    point = ipt.readline()
    while point:
        gene, x, y, value, cell = point.strip().split('\t')
        data.insert(gene, int(x), int(y), value, cell)
        point = ipt.readline()
    return data
...
def rename_out(ipt_file, opt_path):
    ...
    out_cell = opt_path + '/' + prefix + '.cell'
    return out_data, out_index, out_gene, out_cell
...
def main():
    ...
    file = argv[1]
    out_path = argv[2]
    f_data, f_index, f_gene, f_cell = rename_out(file, out_path)
    data = load_data(file)
    load_t = time()
    data.sort()
    sort_t = time()
    ...
    block_t = time()
    data.printf(blocks, blocks_num, f_data, f_index, f_gene, f_cell)
    ed = time()
